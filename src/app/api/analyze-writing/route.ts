...
■ business（ビジネス文書版）
- 「〜でございます」「〜といったものを」「〜における」「〜に関して」は使わない
- 「申し上げます」は文頭の一回のみ許容。それ以降は「〜です」「〜します」「〜だ」で統一
- 格調は語彙の選択で出す。構文を複雑にして格調を出そうとしない
- 「私は〜を信じない」のように主語・述語を短く直結させる
- 抽象名詞（「信用性」「重要性」）を使う場合は必ず動詞に言い換える（「信用しない」「重要だと思わない」）
